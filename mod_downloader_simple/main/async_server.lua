...
--     - Added foreach2 (pairs instead of ipairs)
...
class "_Async" {
    ...
    foreach2 = function(self, array, func, callback)
        self:add(function()
            local a = getTickCount();
            local lastresume = getTickCount();
            ...
            if (self.debug) then
                outputDebugString("[DEBUG]Async foreach2: " .. (getTickCount() - a) .. "ms");
            end
            if (callback) then
                callback();
            end
...
function Async:foreach2(...)
    getInstance():foreach2(...);
end
